from Kurs_cz_129_Praktyczne_zastosowanie_dziedziczenia import BankAccount, MinimumBalanceAccount, MinimumBalanceAccountWithInheritance
...
'''
    Klasa: BankAccount
    Metoda: try_withdraw_with_result_class
    Usprawnienie zwracania rezultatu z użyciem klasy Result dzięki
    dziedziczeniu.
    Stworzenie nowych klas: Ok i Error
    Stan True/False określany na podstawie klas
    Klasy Error i Ok dziedziczą metodę __init__ z klasy wyżej, czyli
    Result.
    Atrybuty message oraz value zostają odziedziczone.
    Wtedy atrybut typu bool w klasie rodzic jako None, brak 
    dopiero klasy dziedziczące (dzieci) ustalają na bieżąco, 
    jaka wartość ma być przypisywana do tego atrybutu.


    def is_ok(self):
        return self.isSuccess
    Metoda w klasie Result, odziedziczona przez Error i Ok,
    zwraca atrybut typu bool względem danej instancji klasy, obiektu: self
    Taki obiekt tutaj to np.: accountMinWithInheritance
    Czytelniejsze:
'''
print("\n\n\n\n\n\n")
accountMinWithInheritance = MinimumBalanceAccountWithInheritance(1500,1000)
